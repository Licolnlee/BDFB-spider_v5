...
CONN1 = RedisClient('url_list', 'myfuture_v6')
    ...
    def req_account(self, headers):
        ...
        try:
            ...
            if response.url == url:
                ...
                content = self.decode_content(html)
                d_url = self.parse_index(content)
                context = self.req_content(d_url)
                filepath = './Cookies/req.html'
                with open(filepath, 'w', encoding = 'utf-8') as f:
                    f.write(context)
                f.close( )
                self.parse_js(filepath)
                return context
            else:
                headers = self.login( )
                print(headers)
    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(cookies)
                self.dict_format('./Cookies/login_cookies.txt', cookies)
                self.txt_reformat('./Cookies/login_cookies.txt')
                headers = self.read_txt_header_update('./Cookies/login_cookies.txt', headers)
                self.headers_json('./Cookies/headers.txt', headers)
                return headers
            else:
                print(response.status_code)
                print(response.history)
